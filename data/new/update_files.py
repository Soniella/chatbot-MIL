...
choice=input('Enter your choice: Type csv or json')
...
if (choice=='csv'):
    ...
    f=open(file='data.txt',mode='w',encoding='utf-8',errors='ignore')
    df_q=pd.read_csv(fname,usecols=["question"])
    df_a=pd.read_csv(fname,usecols=["answer"])

    #print(type(df))
    list_q=df_q.values.tolist()
    list_a=df_a.values.tolist()
    #print(type(a))
    final_q=list_q[:10000]
    final_a=list_a[:10000]

    last_q=[]
    for i in final_q:
        string="".join(i)
        last_q.append(string)
        string=''

    last_a=[]
    for i in final_a:
        string="".join(i)
        last_a.append(string)
        string=''
