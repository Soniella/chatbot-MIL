...
def build_dataset(words, n_words, atleast=1):
    ...
    counter = [i for i in counter if i[1] >= atleast] #To filter out only those tuples which occur more than once.
    ...
    data = list()
    unk_count = 0
    for word in words: #words: list of all words i.e. concat_to
...
for conv in convs: #extracts each list from the list of lists
    for i in range(len(conv)-1): #to abstract from the list
        questions.append(id2line[conv[i]]) #conv[i] gives the id. id2line[conv[i]] gives the string from the dictionary
...
i = 0
#Here choosing only those questions that have appropriate length
...
i = 0
#Here choosing only those answers that have appropriate length from the already filtered answers list
...
'''
#To take only the first response from the user
question_test=[]
...
conn.execute("INSERT INTO USER_ENTRIES (ID,PRONOUN,VERB,NOUN) \
      VALUES (1, pattern[0], pattern[1], pattern[2] )");
"""
...
for i in range(len(short_answers)): #short_answers is the training data
    short_answers[i] += ' EOS'
...
sess.run(tf.global_variables_initializer()) #to initialize ALL variables
...
#Comment all these since X is being initialized later and Y since it is not being used (since we don't want to know real answer.)
...
while(i<100):
    user_response=input("User: ")
    #first make question_test empty else will give predicted output for even the first response again. Make it empty each time.
    question_test.pop()
    question_test.append(user_response)
    print(question_test)
